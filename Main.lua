...
Menu.Banner = {
    enabled = true,
    imageUrl = "https://i.imgur.com/lRbLeTU.png",
    height = 100
}
...
function Menu.LoadBannerTexture(url)
    ...
    if CreateThread then
        CreateThread(function()
            ...
            if not success then
                print("Menu.LoadBannerTexture thread error: " .. tostring(result))
            end
        end)
    else
        local success, result = pcall(function()
        ...
        if not success then
            print("Menu.LoadBannerTexture error: " .. tostring(result))
        end
    end
end
...
function Menu.ApplyTheme(themeName)
    ...
    if themeLower == "red" then
        ...
        Menu.Banner.imageUrl = "https://i.imgur.com/lRbLeTU.png"
    ...
    elseif themeLower == "purple" then
        ...
        Menu.Banner.imageUrl = "https://i.imgur.com/4HqY89K.png"
        Menu.CurrentTheme = "Purple"
    ...
    elseif themeLower == "pink" then
        ...
        Menu.Banner.imageUrl = "https://i.imgur.com/iuabdHk.png"
        Menu.CurrentTheme = "Pink"
    else
        Menu.Colors.HeaderPink = { r = 148, g = 0, b = 211 }
        Menu.Colors.SelectedBg = { r = 148, g = 0, b = 211 }
        Menu.Banner.imageUrl = "https://i.imgur.com/CyteKiU.jpeg"
        Menu.CurrentTheme = "Purple"
    end
...
function Menu.DrawHeader()
    ...
    if Menu.Banner.enabled then
        if Menu.bannerTexture and Menu.bannerTexture > 0 and Susano and Susano.DrawImage then
            ...
            Menu.DrawText(logoX, logoY, "Lojax", 28, 1.0, 1.0, 1.0, 1.0)
        end
    else
        Menu.DrawRect(x, y, width, height, Menu.Colors.HeaderPink.r, Menu.Colors.HeaderPink.g, Menu.Colors.HeaderPink.b, 255)
...
local function calculatePercent(currentValue, minValue, maxValue)
    local range = (maxValue or 0.0) - (minValue or 0.0)
    if range == 0 then
        return 0.0
    end
    local percent = ((currentValue or minValue or 0.0) - (minValue or 0.0)) / range
...
function Menu.DrawItem(x, itemY, width, itemHeight, item, isSelected)
    ...
    if item.type == "toggle" then
        ...
        if item.hasSlider then
            ...
            local percent = calculatePercent(currentValue, minValue, maxValue)
...
function Menu.DrawLoadingBar(alpha)
    ...
    if elapsedTime < 1000 then
        loadingText = "Lojax Injecting"
    elseif elapsedTime < 2000 then
        loadingText = "Lojax Ready"
    else
        loadingText = "Lojax Ready"
    end
...
function Menu.DrawKeybindsInterface(alpha)
    ...
    Menu.DrawText(textX - 1, textY - 1, "Lojax Keybinds", textSize, 0.0, 0.0, 0.0, 1.0 * alpha)
    Menu.DrawText(textX + 1, textY - 1, "Lojax Keybinds", textSize, 0.0, 0.0, 0.0, 1.0 * alpha)
    Menu.DrawText(textX - 1, textY + 1, "Lojax Keybinds", textSize, 0.0, 0.0, 0.0, 1.0 * alpha)
    Menu.DrawText(textX + 1, textY + 1, "Lojax Keybinds", textSize, 0.0, 0.0, 0.0, 1.0 * alpha)
    Menu.DrawText(textX, textY, "Lojax Keybinds", textSize, 1.0, 1.0, 1.0, 1.0 * alpha)
...
function Menu.Render()
    ...
    if Menu.OnRender then
        ...
        if not success then
            print("Menu.OnRender error: " .. tostring(err))
        end
    end
